...
DENSITY_WORKER="$PROJECT_DIR/cellpose_pipeline/hpc/run_density_prepass.sh"
...
PREFLIGHT_ARGS=(
  "$PROJECT_DIR/cellpose_pipeline/scripts/18_run_segmentation_classification_workflow.py"
  --dir "$INPUT_ROOT"
  --recursive
  --profile-mode auto
...
sbatch "${SBATCH_ARGS[@]}" \
  "$PROJECT_DIR/cellpose_pipeline/hpc/run_separate_images_array_task.sh" \
