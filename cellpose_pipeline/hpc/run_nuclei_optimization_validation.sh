...
OUT_DIR="${OUT_DIR:-$RESULTS_ROOT/nuclei_optimization_validation_hpc_20260710}"
...
export MPLCONFIGDIR="${TMPDIR:-/tmp}/nuclei_validation_mpl_${SLURM_JOB_ID:-manual}"
...
"$PYTHON_BIN" cellpose_pipeline/scripts/32_tune_nuclei_segmentation.py \
  --input-root "$INPUT_ROOT" \
  --baseline-run "$BASELINE_RUN" \
  --out-dir "$OUT_DIR" \
  --all-images \
  --inference-tags pct0p5_99p9_d24 \
  --mask-tags cp-1p0_f0_min15 cp-2p0_f0_min15 \
  --use-gpu \
  --save-previews \
  --save-masks

echo "nuclei_optimization_validation_complete=1"
