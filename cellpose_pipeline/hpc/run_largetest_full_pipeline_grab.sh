...
stage_audit() {
  "$PYTHON_BIN" -I "$SCRIPT_DIR/44_audit_largetest_full_pipeline.py" \
    --input-root "$INPUT_ROOT" \
    --run-root "$OUT_ROOT" \
...
sha256sum \
  "$SCRIPT_DIR/18_run_segmentation_classification_workflow.py" \
  "$SCRIPT_DIR/28_call_high_density_from_nuclei_masks.py" \
  "$SCRIPT_DIR/29_fuse_multichannel_classification.py" \
  "$SCRIPT_DIR/36_screen_shape_aware_nucleus_splits.py" \
  "$SCRIPT_DIR/38_render_shape_aware_nucleus_split_qc.py" \
  "$SCRIPT_DIR/42_merge_shape_strict_shards.py" \
  "$SCRIPT_DIR/43_finalize_shape_strict_qc.py" \
  "$SCRIPT_DIR/44_audit_largetest_full_pipeline.py" \
  "$HPC_DIR/run_largetest_full_pipeline_grab.sh" \
