...
BASE="${BASE:-/share/lab_crd/lab_crd/HighPloidy_CostBenefits/data/BreastCancerCellLines/SUM-159/N01_Incucyte_SUM159_Doxorubicin_Test1}"
...
stage_environment() {
  ...
  "$PYTHON_BIN" -I "$SCRIPT_DIR/18_run_segmentation_classification_workflow.py" \
...
stage_density() {
  "$PYTHON_BIN" -I "$SCRIPT_DIR/28_call_high_density_from_nuclei_masks.py" \
...
stage_dead_consensus() {
  ...
  while IFS= read -r dead_image; do
    ...
    "$PYTHON_BIN" -I "$SCRIPT_DIR/49_segment_dead_with_combined_blue_consensus.py" \
  ...
  "$PYTHON_BIN" -I "$SCRIPT_DIR/50_merge_dead_consensus_outputs.py" --dead-run "$OUT_ROOT/Dead" --expected-keys "$TASK_LIST"
  "$PYTHON_BIN" -I "$SCRIPT_DIR/51_render_dead_consensus_qc.py" --input-root "$INPUT_ROOT" --dead-run "$OUT_ROOT/Dead" --out-dir "$OUT_ROOT/Dead/qc"
...
run_fusion() {
  ...
  "$PYTHON_BIN" -I "$SCRIPT_DIR/29_fuse_multichannel_classification.py" \
...
stage_nucleated_branch() {
  "$PYTHON_BIN" -I "$SCRIPT_DIR/47_build_nucleated_cell_branch.py" \
...
sha256sum "$SCRIPT_DIR/18_run_segmentation_classification_workflow.py" "$SCRIPT_DIR/45_calibrate_dead_combined_blue.py" \
  "$SCRIPT_DIR/47_build_nucleated_cell_branch.py" "$SCRIPT_DIR/49_segment_dead_with_combined_blue_consensus.py" \
  "$SCRIPT_DIR/50_merge_dead_consensus_outputs.py" "$HPC_DIR/run_largetest_calibrated_dead_dual_branch_grab.sh" \
