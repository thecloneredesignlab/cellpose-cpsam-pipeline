#!/usr/bin/env python3
"""Build one immutable multimodal-cell-state V3 feature generation."""
...
HELPER_PATH = SCRIPT_DIR / "_shared" / "multimodal_cell_state_v3_features.py"
SPEC = importlib.util.spec_from_file_location(
    "multimodal_cell_state_v3_features_local", HELPER_PATH
)
if SPEC is None or SPEC.loader is None:
    raise ImportError(f"Cannot load V3 feature helper: {HELPER_PATH}")
...
def load_contract(
    ...
    for path in (
        ...
        if not path.is_file() or path.is_symlink():
            raise FileNotFoundError(
                f"Required V3 input is unavailable or a symlink: {path}"
            )
    ...
    if configuration.get("schema_version") != CONFIG_SCHEMA_VERSION:
        raise ValueError("Unsupported multimodal-cell-state V3 config")
    if configuration.get("method_version") != "sum159_multimodal_cell_state_v3":
        raise ValueError("V3 config method_version differs")
    profiles = payload.get("profiles")
    if not isinstance(profiles, dict):
        raise ValueError("Field record lacks profiles")
    nuclei = profiles.get("Nuclei")
    combined = profiles.get("Combined")
    if not isinstance(nuclei, dict) or not isinstance(combined, dict):
        raise ValueError("Field record lacks Nuclei or Combined profile")
    mask_field = next(
        (
            name
    ...
    return {
        ...
        "paths": paths,
        "minimum_contrast": float(miss_gate["minimum_cell_to_ring_contrast_mad"]),
        "minimum_positive_fraction": float(miss_gate["minimum_positive_fraction"]),
    }
...
def read_broad(path: Path, key: str) -> list[dict[str, str]]:
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        if missing:
            raise ValueError(f"Broad feature shard lacks V3 inputs: {missing}")
        rows = list(reader)
    if not rows or any(
        row["key"] != key or row["branch"] != "original" for row in rows
    ):
        raise ValueError(
            "Broad feature shard key/branch differs from V3 field contract"
        )
...
def read_selected_cells(path: Path, key: str, well: str) -> dict[str, int]:
    with path.open(newline="", encoding="utf-8") as handle:
        ...
        if missing:
            raise ValueError(
                f"Selected-cell table lacks V3 identity columns: {sorted(missing)}"
            )
...
def artifact_hashes(directory: Path) -> dict[str, str]:
    ...
    if any(path.is_symlink() for path in files):
        raise ValueError("V3 feature generation contains a symlink")
...
def verify_existing(output: Path, inputs: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise ValueError("Existing V3 feature generation is partial")
    receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
    if (
        receipt.get("schema_version") != SCHEMA_VERSION
        or receipt.get("status") != "COMPLETE"
    ):
        raise ValueError("Existing V3 feature receipt is not COMPLETE")
    if receipt.get("inputs") != inputs:
        raise ValueError("Existing V3 feature input identity differs")
    declared = receipt.get("output_file_sha256")
    observed = {"features.tsv": sha256_file(feature_path)}
    if declared != observed:
        raise ValueError("Existing V3 feature artifact hash differs")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if output.exists():
        if output.is_symlink() or not output.is_dir():
            raise ValueError(f"V3 feature output is not a real directory: {output}")
        if not args.overwrite:
            raise FileExistsError(output)
        verify_existing(output, expected_inputs(contract))
        print(f"v3_feature_generation={output}")
    ...
    nuclei_raw = FEATURES.scalar_first_plane(
        load_first_tiff_page(contract["paths"]["nuclei_raw"]), label="Nuclei raw"
    )
    ...
    if nuclei_raw.shape != nuclei_mask.shape or nuclei_raw.shape != combined_mask.shape:
        raise ValueError(
            f"V3 field shapes differ: raw={nuclei_raw.shape} nuclei={nuclei_mask.shape} combined={combined_mask.shape}"
        )
    ...
    for broad in broad_rows:
        ...
        row.update(
            FEATURES.nucleus_features_for_object(
                ...
                nuclei_mask[bounds],
                nuclei_raw[bounds],
                minimum_contrast=contract["minimum_contrast"],
                minimum_positive_fraction=contract["minimum_positive_fraction"],
            )
        )
        if set(row) != set(OUTPUT_COLUMNS):
            raise RuntimeError(
                f"Internal V3 output schema differs: missing={sorted(set(OUTPUT_COLUMNS)-set(row))}"
            )
        rows.append(row)
    ...
    try:
        ...
        statuses: dict[str, int] = {name: 0 for name in FEATURES.MEASUREMENT_STATUSES}
        for row in rows:
            statuses[str(row["nuclei_measurement_status"])] += 1
        receipt = {
            ...
            "nuclei_measurement_status_counts": statuses,
            "inputs": inputs,
        ...
        if output.exists():
            raise FileExistsError(
                f"V3 feature output appeared during staging: {output}"
            )
