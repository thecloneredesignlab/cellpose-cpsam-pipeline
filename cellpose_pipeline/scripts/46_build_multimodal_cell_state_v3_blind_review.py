#!/usr/bin/env python3
"""Freeze a 500-cell V3 blind review set without exposing UMAP-derived labels."""
...
SCHEMA_VERSION = "multimodal_cell_state_v3_blind_review_v1"
IDENTITY_SCHEMA_VERSION = "multimodal_cell_state_v3_blind_review_generation_identity_v1"
...
def select_blind(
    ...
    if total > max_per_well * len(wells):
        raise ValueError("Blind-review total exceeds well-capacity contract")
    base, remainder = divmod(total, len(wells))
    if base > max_per_well or base + bool(remainder) > max_per_well:
        raise ValueError("Blind-review well cap cannot support balanced allocation")
    priority = sorted(
    ...
    if len(selected) != total or len({row["cell_id"] for row in selected}) != total:
        raise RuntimeError(
            "Blind-review selection did not produce the exact unique total"
        )
...
def directory_hashes(root: Path, exclude: set[str] | None = None) -> dict[str, str]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            raise ValueError(f"V3 blind-review generation contains a symlink: {path}")
...
def verify_existing(
    ...
    manifest_path = output / "blind_review_manifest.json"
    identity_path = output / "blind_review_generation_identity.json"
    if not manifest_path.is_file() or not identity_path.is_file():
        raise ValueError("Existing V3 blind-review generation is partial")
    ...
    if (
        ...
        raise ValueError("Existing V3 blind-review identity differs")
    observed = directory_hashes(
        output, {"blind_review_manifest.json", "blind_review_generation_identity.json"}
    )
    if manifest.get("output_file_sha256") != observed:
        raise ValueError("Existing V3 blind-review artifact set/hash differs")
    expected = {
    ...
    if load_json(identity_path) != expected:
        raise ValueError("Existing V3 blind-review generation identity differs")


def main(argv: Sequence[str] | None = None) -> int:
    args = parse_args(argv)
    if args.max_total < 1 or args.max_per_well < 1:
        raise ValueError("Blind-review size/cap must be positive")
    ...
    if shadow.is_symlink() or not shadow.is_dir() or shadow not in project_path.parents:
        raise ValueError("V3 project/shadow-root identity differs")
    if output == shadow or shadow not in output.parents:
        raise ValueError("V3 blind-review output must remain inside its shadow root")
    project = load_json(project_path)
    if project.get("project_id") != PROJECT_ID:
        raise ValueError("Unsupported V3 project")
    ...
    for path in (
        ...
        if not path.is_file() or path.is_symlink():
            raise ValueError(f"V3 blind-review input is unavailable: {path}")
    ...
    if [row["cell_id"] for row in coordinates] != ids or [
        ...
        raise ValueError("V3 blind-review input row universes differ")
    joined = [
        {
            **cell,
    ...
    if any(row.get("split") != "development" for row in joined):
        raise ValueError("Heldout cells entered V3 blind review")
    selected = select_blind(joined, args.max_total, args.max_per_well, args.seed)
    review_rows = [
        {
            "morphology_umap_row_key": row["cell_id"],
            "review_default_label": "uncertain",
            "review_sampling_bucket": "multimodal_v3_blind",
    ...
    if output.exists():
        ...
        print(f"multimodal_cell_state_v3_blind_review={output}")
    ...
    try:
        review_path = staging / "blind_review_set.tsv"
        write_tsv(review_path, review_rows)
        well_counts = Counter(row["source_id"] for row in selected)
        status_counts = Counter(row["nuclei_measurement_status"] for row in selected)
        ...
        manifest = {
            ...
            "selection": {
                ...
                "stratification_audit": "well_by_diagnostic_cluster_by_nuclei_measurement_status",
                "within_stratum": "deterministic_umap_farthest_point",
            },
            "diagnostic_cluster_counts": dict(sorted(cluster_counts.items())),
            "nuclei_measurement_status_counts": dict(sorted(status_counts.items())),
            "review_default_label": "uncertain",
        ...
        manifest_path = staging / "blind_review_manifest.json"
        ...
        (staging / "blind_review_generation_identity.json").write_text(
            json.dumps(identity, indent=2, sort_keys=True) + "\n", encoding="utf-8"
        )
        if output.exists():
            raise FileExistsError(
                f"V3 blind-review output appeared during staging: {output}"
            )
