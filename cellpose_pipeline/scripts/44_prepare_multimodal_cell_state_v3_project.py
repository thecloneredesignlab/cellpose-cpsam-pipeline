#!/usr/bin/env python3
"""Create an immutable CPA exploration project for the selected V3 geometry.

The CPA UMAP remains an exploratory view.  V3 training accepts only labels from
the separate blind exact-review generation; polygon labels from this project
are not a training input.
"""
...
SCHEMA_VERSION = "multimodal_cell_state_v3_project_import_v1"
IDENTITY_SCHEMA_VERSION = "multimodal_cell_state_v3_project_generation_identity_v1"
PROJECT_SCHEMA_VERSION = "cell_phenotype_annotator_project_v1"
PROJECT_ID = "multimodal_cell_state_v3_development"
PROJECTION_SCHEMA = "multimodal_cell_state_v3_projection_v1"
MERGE_SCHEMA = "multimodal_cell_state_v3_feature_merge_v1"
...
def artifact_hashes(root: Path, exclude: set[str] | None = None) -> dict[str, str]:
    ...
    for path in sorted(root.rglob("*")):
        if path.is_symlink():
            raise ValueError(f"V3 project contains a symlink: {path}")
        if path.is_file():
...
def verify_existing(
    ...
    if not manifest_path.is_file() or not identity_path.is_file():
        raise ValueError("Existing V3 project is partial")
    ...
    if (
        ...
        raise ValueError("Existing V3 project identity differs")
    ...
    if manifest.get("output_file_sha256") != observed:
        raise ValueError("Existing V3 project artifact set/hash differs")
    expected = {
    ...
    if load_json(identity_path) != expected:
        raise ValueError("Existing V3 project generation identity differs")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    feature_root = require_inside(
        require_dir(args.feature_merge, "V3 feature merge"),
        shadow_root,
        "V3 feature merge",
    )
    projection_root = require_inside(
        require_dir(args.projection, "V3 projection"), shadow_root, "V3 projection"
    )
    output = (
        args.output_dir.expanduser().resolve()
        if args.output_dir
        else shadow_root / "projection_input" / "multimodal_cell_state_v3"
    )
    if output == shadow_root or shadow_root not in output.parents:
        raise ValueError("V3 project output must remain inside its shadow root")
    if any(
        root == output or root in output.parents or output in root.parents
        for root in (feature_root, projection_root)
    ):
        raise ValueError(
            "V3 project output must not overlap feature/projection evidence"
        )
    ...
    if not required_cells.issubset(cell_fields):
        raise ValueError(
            f"Base cells lack V3 identity: {sorted(required_cells-set(cell_fields))}"
        )
    ...
    if (
        ...
        raise ValueError("Base V3 development-cell universe differs")
    image_fields, images = read_tsv(base_assets["images_file"])
    if not {"image_id", "channel_id"}.issubset(image_fields) or {
        row["channel_id"] for row in images
    } != set(EXPECTED_CHANNELS):
        raise ValueError("Base image registry is not the blind Brightfield/Nuclei pair")
    class_fields, classes = read_tsv(base_assets["classes_file"])
    if (
        "class_id" not in class_fields
        or tuple(row["class_id"] for row in classes) != EXPECTED_CLASSES
    ):
        raise ValueError("Base class ontology differs")

    merge_manifest_path = require_file(
        feature_root / "feature_merge_manifest.json", "V3 feature merge manifest"
    )
    merge_features_path = require_file(
        feature_root / "features.tsv", "V3 merged features"
    )
    ...
    if (
        ...
        raise ValueError("V3 feature merge manifest is incomplete")
    if merge_manifest.get("output_file_sha256", {}).get("features.tsv") != sha256_file(
        merge_features_path
    ):
        raise ValueError("V3 merged feature hash differs")
    feature_fields, feature_rows = read_tsv(merge_features_path)
    if [row["cell_id"] for row in feature_rows] != cell_ids:
        raise ValueError("V3 merged features are not in base cell lockstep")

    projection_manifest_path = require_file(
        projection_root / "projection_manifest.json", "V3 projection manifest"
    )
    decision_path = require_file(
        projection_root / "calibration_decision.json", "V3 calibration decision"
    )
    umap_path = require_file(projection_root / "umap.tsv", "V3 selected UMAP")
    clusters_path = require_file(
        projection_root / "diagnostic_clusters.tsv", "V3 diagnostic clusters"
    )
    ...
    if (
        decision.get("representation_gate") != "GO_TO_BLIND_LABEL_MAPPING"
        or decision.get("dead_cell_island_required") is not False
    ):
        raise ValueError(
            "V3 representation has not passed its blind label-mapping gate"
        )
    ...
    for path in (decision_path, umap_path, clusters_path):
        ...
        if declared_projection.get(relative) != sha256_file(path):
            raise ValueError(f"V3 projection artifact hash differs: {relative}")
    ...
    if [row["cell_id"] for row in umap_rows] != cell_ids or [
        ...
        raise ValueError("V3 UMAP/cluster rows differ from the cell universe")
    ...
    if (
        ...
        raise ValueError("V3 projection selected feature columns are invalid")
    ...
    if output.exists():
        if output.is_symlink() or not output.is_dir():
            raise ValueError("Existing V3 project must be a real directory")
        if not args.overwrite:
            raise FileExistsError(output)
        verify_existing(output, inputs, implementation_sha)
        print(f"multimodal_cell_state_v3_project={output / 'project.yml'}")
    ...
    try:
        ...
        for name in ("diagnostic_cluster", "nuclei_measurement_status"):
        ...
        updated_cells = [
            {
                **row,
                ...
                "nuclei_measurement_status": statuses[row["cell_id"]],
            }
        ...
        project = {
            ...
            "annotation": {
                "title": "SUM159 multimodal cell-state V3 exploration (blind exact review is authoritative)",
        ...
        manifest = {
            ...
            "training_label_source": "separate_blind_exact_review_only",
        ...
        if sha256_file(implementation) != implementation_sha:
            raise RuntimeError("V3 project builder changed during execution")
        if output.exists():
            raise FileExistsError(
                f"V3 project output appeared during staging: {output}"
            )
