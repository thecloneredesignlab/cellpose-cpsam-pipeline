#!/usr/bin/env python3
"""Run the resumable direct-node V3 calibration to its blind-review barrier."""
...
SCHEMA_VERSION = "multimodal_cell_state_v3_calibration_receipt_v1"
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.concurrency < 1 or args.expected_cell_count < 1:
        raise ValueError("Invalid V3 calibration concurrency/cell count")
    ...
    shadow = require_dir(args.shadow_root, "V3 shadow root")
    ...
    if any(
        ...
        raise ValueError("V3 shadow root must be independent from frozen parent roots")
    ...
    config = require_file(
        ...
        / "cellpose_pipeline"
        / "configs"
        / "multimodal_cell_state_v3.json",
        "V3 config",
    )
    extractor = require_file(
        scripts / "41_extract_multimodal_cell_state_v3_features.py", "field extractor"
    )
    helper = require_file(
        scripts / "_shared" / "multimodal_cell_state_v3_features.py", "feature helper"
    )
    merger = require_file(
        scripts / "42_merge_multimodal_cell_state_v3_features.py", "feature merger"
    )
    projector = require_file(
        scripts / "43_build_multimodal_cell_state_v3_projection.R", "projection builder"
    )
    project_builder = require_file(
        scripts / "44_prepare_multimodal_cell_state_v3_project.py", "project builder"
    )
    audit_renderer = require_file(
        scripts / "45_render_multimodal_cell_state_v3_audit.py", "audit renderer"
    )
    blind_builder = require_file(
        scripts / "46_build_multimodal_cell_state_v3_blind_review.py",
        "blind-review builder",
    )
    exact_renderer = require_file(
        scripts / "37_render_reference_cell_state_exact_review.py",
        "exact-review renderer",
    )
    ...
    if any(row["split"] != "development" for row in cells):
        raise ValueError("Heldout cells entered V3 calibration")
    field_keys = sorted({(row["well"], row["key"]) for row in cells})
    feature_root = shadow / "workflow_status" / "multimodal_v3_field_features"
    ...
    def extract_field(item: tuple[str, str]) -> tuple[str, str]:
        ...
        return key, run(command, f"V3 field extraction {key}")
    ...
    with ThreadPoolExecutor(max_workers=args.concurrency) as executor:
        ...
        for future in as_completed(futures):
            ...
            if completed % 100 == 0 or completed == len(futures):
                print(f"v3_field_features={completed}/{len(futures)}", flush=True)

    merge_root = shadow / "workflow_status" / "multimodal_v3_feature_merge"
    run(
        [
            sys.executable,
            "-I",
    ...
    projection_root = shadow / "workflow_status" / "multimodal_v3_projection"
    run(
        [
            "Rscript",
            str(projector),
    ...
    blind_root = shadow / "human_review" / "blind_seed1" / "selection"
    run(
        [
            sys.executable,
            "-I",
        ...
        "V3 blind-review selection",
    )
    render_root = shadow / "human_review" / "blind_seed1" / "render"
    run(
        [
            sys.executable,
            "-I",
            ...
            str(project_file),
            "--review-set",
            str(blind_root / "blind_review_set.tsv"),
            "--review-manifest",
            str(blind_root / "blind_review_manifest.json"),
            "--output-dir",
            str(render_root),
            "--padding",
            "12",
        ],
        "V3 blind-review render",
    )

    decision = load_json(projection_root / "calibration_decision.json")
    blind_manifest = load_json(blind_root / "blind_review_manifest.json")
    render_manifest = load_json(render_root / "exact_review_render_manifest.json")
    if decision.get("representation_gate") != "GO_TO_BLIND_LABEL_MAPPING":
        raise ValueError("V3 representation did not pass block-equalization gate")
    if (
        blind_manifest.get("row_count") != 500
        or render_manifest.get("all_crops_available") is not True
    ):
        raise ValueError("V3 blind-review barrier is incomplete")
    ...
    receipt_path = (
        shadow
        / "workflow_status"
        / "MULTIMODAL_CELL_STATE_V3_CALIBRATION_COMPLETE.json"
    )
    completed_utc = (
    ...
    if not completed_utc:
        raise ValueError("Existing V3 calibration receipt lacks completed_utc")
    receipt = {
        ...
        "blind_review_rows": blind_manifest["row_count"],
        "blind_review_labels_displayed": False,
        "human_barrier": "blind_exact_review_submission_required",
        "human_workspace": str(render_root / "exact_review.html"),
        "human_submission_expected_path": str(
            render_root / "exact_review_submission.json"
        ),
        "inputs": {
            "base_project": {
        ...
        "artifacts": {
            ...
            "blind_review_manifest": {
                "path": str(blind_root / "blind_review_manifest.json"),
                "sha256": sha256_file(blind_root / "blind_review_manifest.json"),
            },
    ...
    print("multimodal_cell_state_v3_calibration_complete=1")
