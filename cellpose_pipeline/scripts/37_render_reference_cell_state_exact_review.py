...
body{{font-family:Arial;margin:16px}}header{{position:sticky;top:0;background:#fff;padding:8px;border-bottom:1px solid #aaa;z-index:2}}#tiles{{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:12px}}article{{border:1px solid #bbb;padding:8px}}.images{{display:grid;grid-template-columns:1fr 1fr;gap:4px;background:#222}}img{{width:100%;height:180px;object-fit:contain}}label{{display:block;margin:5px 0}}select,textarea,input{{width:100%;box-sizing:border-box}}code{{font-size:9px;overflow-wrap:anywhere}}
...
"use strict";const p=JSON.parse(document.getElementById("data").textContent);const $=x=>document.getElementById(x);const state=p.rows.map(x=>({{...x,label:x.review_default_label||"uncertain",confidence:"high",notes:"",reviewer:"",reviewed_at:""}}));
function render(){{$("summary").textContent=`${{state.length}} exact preselected cells; no secondary sampling`;const root=$("tiles");root.replaceChildren();state.forEach((r,i)=>{{const a=document.createElement("article");a.innerHTML=`<div class="images"><img src="${{r.brightfield}}" alt="Brightfield"><img src="${{r.nuclei}}" alt="Nuclei support"></div><p>${{r.review_sampling_bucket}} | ${{r.context_key}} | ${{r.source_id}}</p><code>${{r.stable_id}}</code><label>Manual label<select class="label">${{p.classes.map(x=>`<option value="${{x}}" ${{x===r.label?"selected":""}}>${{x}}</option>`).join("")}}</select></label><label>Confidence<select class="confidence"><option>high</option><option>medium</option><option>low</option></select></label><label>Notes<textarea class="notes"></textarea></label>`;a.querySelector(".label").onchange=e=>r.label=e.target.value;a.querySelector(".confidence").onchange=e=>r.confidence=e.target.value;a.querySelector(".notes").oninput=e=>r.notes=e.target.value;root.appendChild(a);}})}}
...
$("export").onclick=()=>{{const reviewer=$("reviewer").value.trim();if(!reviewer){{$("status").textContent="Reviewer required";return}}const stamp=new Date().toISOString();download({{schema_version:p.submission_schema_version,identity:p.identity,creation_metadata:{{created_at:stamp,client_version:"reference-cell-state-v2-exact-review"}},rows:state.map(r=>({{morphology_umap_row_key:r.stable_id,label:r.label,label_confidence:r.confidence,reviewer:reviewer,reviewed_at:stamp,review_notes:r.notes}}))}});$("status").textContent="Submission downloaded"}};render();
</script></body></html>'''
