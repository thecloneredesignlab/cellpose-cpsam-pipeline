#!/usr/bin/env python3
"""Render one exact historical preselection without any second-stage sampling."""
...
CLASS_IDS = ("live_cell", "dead_cell", "multinucleated_cell", "uncertain")
...
def verify_render_generation(
    ...
    if fields != expected_fields or len(crops) != 2 * len(expected_ids):
    ...
    expected_pairs = {(stable_id, channel) for stable_id in expected_ids for channel in ("brightfield", "nuclei_support")}
...
def normalize(array: np.ndarray, low: float, high: float) -> np.ndarray:
...
def render_assets(
    ...
    for rank, row in enumerate(selected, 1):
        ...
        if bf_row is None or nuclei_row is None:
            raise ValueError(f"Missing BF/Nuclei rows: {image_id}")
        if image_id not in cache:
            ...
            if mask_path.resolve() != Path(nuclei_row["mask_path"]).resolve():
                raise ValueError(f"BF/Nuclei mask identity mismatch: {image_id}")
            mask = read_plane(mask_path, "", bf_row.get("mask_channel_index", ""), height, width)
            cache[image_id] = (bf, nuclei, mask)
        bf, nuclei, mask = cache[image_id]
        ...
        bf_scaled = normalize(bf, float(bf_row.get("display_percentile_low") or 1), float(bf_row.get("display_percentile_high") or 99))
        nuclei_scaled = normalize(nuclei, float(nuclei_row.get("display_percentile_low") or 1), float(nuclei_row.get("display_percentile_high") or 99))
        ...
        status.append({
            ...
            "mask_label": str(label), "brightfield_status": "ok", "nuclei_status": "ok",
...
<title>Reference cell-state exact review</title><style>
body{{font-family:Arial;margin:16px}}header{{position:sticky;top:0;background:#fff;padding:8px;border-bottom:1px solid #aaa;z-index:2}}#guide{{background:#f5f7fa;border:1px solid #ccd3dc;padding:10px;margin:8px 0}}#tiles{{display:grid;grid-template-columns:repeat(auto-fill,minmax(300px,1fr));gap:12px}}article{{border:1px solid #bbb;padding:8px}}article.reviewed{{border:2px solid #16875b}}.images{{display:grid;grid-template-columns:1fr 1fr;gap:4px;background:#222}}img{{width:100%;height:180px;object-fit:contain}}label{{display:block;margin:5px 0}}select,textarea,input{{width:100%;box-sizing:border-box}}input[type=checkbox]{{width:auto}}code{{font-size:9px;overflow-wrap:anywhere}}button{{margin-right:8px}}#status{{font-weight:bold}}
</style></head><body><header><h1>Reference cell-state exact review</h1><p id="summary"></p><label>Reviewer<input id="reviewer"></label><button id="export">Export authoritative submission</button><span id="status"></span></header><main id="tiles"></main>
<script type="application/json" id="data">{payload_json}</script><script>
"use strict";const p=JSON.parse(document.getElementById("data").textContent);const $=x=>document.getElementById(x);const storageKey=`reference-cell-state-v2:${{p.identity.render_generation_id}}`;const empty=()=>p.rows.map(x=>({{...x,label:x.review_default_label||"uncertain",confidence:"high",notes:"",reviewed:false}}));let state=empty();
...
function restore(){{try{{const saved=JSON.parse(localStorage.getItem(storageKey)||"null");if(!saved||!Array.isArray(saved.state)||saved.state.length!==p.rows.length)return;if(!saved.state.every((r,i)=>r.stable_id===p.rows[i].stable_id))return;state=saved.state;$("reviewer").value=saved.reviewer||""}}catch(e){{state=empty()}}}}
function updateSummary(){{const reviewed=state.filter(r=>r.reviewed).length;const counts={{live_cell:0,dead_cell:0,multinucleated_cell:0,uncertain:0}};state.forEach(r=>{{counts[r.label]=(counts[r.label]||0)+1}});$("summary").textContent=`Reviewed ${{reviewed}}/${{state.length}} | live ${{counts.live_cell||0}} | dead ${{counts.dead_cell||0}} | multinucleated ${{counts.multinucleated_cell||0}} | uncertain ${{counts.uncertain||0}}`;}}
function render(){{const header=document.querySelector("header");const guide=document.createElement("div");guide.id="guide";guide.innerHTML='<b>Morphology-only guide (no Dead channel, current classifier, UMAP, or cluster is shown)</b><br>Brightfield is on the left; nuclei support is on the right. Choose <b>live_cell</b> only for clearly intact/spread cells; <b>dead_cell</b> only for clearly rounded, collapsed, or fragmented death-like morphology; <b>multinucleated_cell</b> only when at least two distinct nuclei are supported inside the same Combined-mask object; otherwise choose <b>uncertain</b>. High/medium confidence may become training evidence; low confidence and uncertain are excluded.';header.insertBefore(guide,$("reviewer"));const clear=document.createElement("button");clear.textContent="Clear saved draft";clear.onclick=()=>{{if(!confirm("Clear all saved labels and review progress for this exact review generation?"))return;localStorage.removeItem(storageKey);state=empty();$("reviewer").value="";document.getElementById("tiles").replaceChildren();renderTiles();save()}};header.insertBefore(clear,$("status"));renderTiles();}}
function renderTiles(){{const root=$("tiles");root.replaceChildren();state.forEach((r,i)=>{{const a=document.createElement("article");if(r.reviewed)a.classList.add("reviewed");a.innerHTML=`<div class="images"><img src="${{r.brightfield}}" alt="Brightfield"><img src="${{r.nuclei}}" alt="Nuclei support"></div><p>${{r.review_sampling_bucket}} | ${{r.context_key}} | ${{r.source_id}}</p><code>${{r.stable_id}}</code><label>Manual label<select class="label">${{p.classes.concat(["uncertain"]).filter((x,j,a)=>a.indexOf(x)===j).map(x=>`<option value="${{x}}" ${{x===r.label?"selected":""}}>${{x}}</option>`).join("")}}</select></label><label>Confidence<select class="confidence"><option ${{r.confidence==="high"?"selected":""}}>high</option><option ${{r.confidence==="medium"?"selected":""}}>medium</option><option ${{r.confidence==="low"?"selected":""}}>low</option></select></label><label>Notes<textarea class="notes"></textarea></label><label><input type="checkbox" class="reviewed" ${{r.reviewed?"checked":""}}> I inspected this cell</label>`;a.querySelector(".notes").value=r.notes||"";a.querySelector(".label").onchange=e=>{{r.label=e.target.value;save();updateSummary()}};a.querySelector(".confidence").onchange=e=>{{r.confidence=e.target.value;save()}};a.querySelector(".notes").oninput=e=>{{r.notes=e.target.value;save()}};a.querySelector(".reviewed").onchange=e=>{{r.reviewed=e.target.checked;a.classList.toggle("reviewed",r.reviewed);save();updateSummary()}};root.appendChild(a);}});updateSummary()}}
function download(value){{const blob=new Blob([JSON.stringify(value,null,2)+"\\n"],{{type:"application/json"}});const u=URL.createObjectURL(blob);const a=document.createElement("a");a.href=u;a.download="exact_review_submission.json";a.click();setTimeout(()=>URL.revokeObjectURL(u),0)}}
$("reviewer").oninput=save;$("export").onclick=()=>{{const reviewer=$("reviewer").value.trim();if(!reviewer){{$("status").textContent="Reviewer required";return}}const remaining=state.filter(r=>!r.reviewed).length;if(remaining){{$("status").textContent=`${{remaining}} cells still require explicit inspection`;return}}const stamp=new Date().toISOString();download({{schema_version:p.submission_schema_version,identity:p.identity,creation_metadata:{{created_at:stamp,client_version:"reference-cell-state-v2-exact-review-guided"}},rows:state.map(r=>({{morphology_umap_row_key:r.stable_id,label:r.label,label_confidence:r.confidence,reviewer:reviewer,reviewed_at:stamp,review_notes:r.notes}}))}});$("status").textContent="Submission downloaded";save()}};restore();render();
</script></body></html>'''
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if project_candidate.parent.parent.name != "projection_input":
        raise ValueError("Project must be under REFERENCE_SHADOW_ROOT/projection_input/<project>/project.yml")
    ...
    try:
        rendered, status = render_assets(selected, cells, image_rows, staging, args.padding)
        ...
        for row in selected:
            ...
            payload_rows.append({
                ...
                "review_sampling_bucket": row["review_sampling_bucket"],
                "context_key": row["context_key"], "source_id": row["source_id"],
                "brightfield": asset["brightfield"], "nuclei": asset["nuclei"],
            })
        ...
        for row in rendered:
            crop_rows.extend((
                ...
                {
                    "morphology_umap_row_key": row["stable_id"], "channel": "nuclei_support",
        ...
        payload = {
            ...
            "identity": identity, "classes": list(CLASS_IDS), "rows": payload_rows,
        }
        ...
        render_manifest = {
            ...
            "channels": ["brightfield", "nuclei_support"], "localization_anchor": "combined_mask",
