...
# Shape/Brightfield/Nuclei block geometry for the SUM159 data available here.

options(stringsAsFactors = FALSE, warn = 1)

SCHEMA_VERSION <- "multimodal_cell_state_v3_projection_v1"
IDENTITY_SCHEMA_VERSION <- "multimodal_cell_state_v3_projection_generation_identity_v1"
CONFIG_SCHEMA_VERSION <- "multimodal_cell_state_v3_config_v1"
...
PRIMARY_PROFILE <- "v3_pruned_block_equal"
GENERATED_PROFILES <- c(
  "v3_zero_aware_unpruned", "v3_pruned_unbalanced", PRIMARY_PROFILE,
  "v3_pruned_block_equal_plus_nuclei_signal_qc", "v3_minus_shape",
  "v3_minus_brightfield", "v3_minus_nuclei"
)
...
current_script_path <- function() {
  ...
  if (length(argument) != 1L) fail("Could not resolve V3 projection implementation path")
...
validate_config <- function(path) {
  ...
  if (!identical(value$schema_version, CONFIG_SCHEMA_VERSION) ||
      !identical(value$method_version, "sum159_multimodal_cell_state_v3")) {
    fail("Unsupported multimodal-cell-state V3 config")
  }
  ...
  if (!identical(as.character(as_vector(blocks$shape$candidate_columns, "shape features")), SHAPE) ||
    ...
    fail("V3 block feature order differs")
  }
  equal <- value$block_equalization
  if (!identical(as.character(as_vector(equal$blocks, "equal blocks")), c("shape", "brightfield", "nuclei")) ||
      abs(as.numeric(equal$target_fraction_each) - 1 / 3) > 1e-12) {
    fail("V3 block equalization contract differs")
  }
  profiles <- as.character(as_vector(value$projection_profiles, "projection profiles"))
  if (!identical(profiles, c("expanded39_audit", GENERATED_PROFILES))) fail("V3 projection profile order differs")
  ...
  if (!identical(grid$primary_profile, PRIMARY_PROFILE) ||
    ...
    fail("V3 projection grid differs or is invalid")
  }
  if (!identical(value$projection_audit$dead_island_required, FALSE)) fail("V3 must not require a dead-cell UMAP island")
  forbidden <- as.character(as_vector(value$blinding$forbidden_before_model_freeze, "forbidden inputs"))
  if (!all(c("Dead", "current_classification", "trajectory", "heldout_cells") %in% forbidden)) {
    fail("V3 blinding contract is incomplete")
  }
  value
}
...
transform_kind <- function(name) {
  ...
  if (name %in% c("roundness", "extent", "solidity", "nuclei_area_fraction_if_present",
                  ...
                  "nuclei_signal_positive_fraction", "bf_edge_density")) return("logit_clipped")
  if (name %in% c("aspect_ratio", "bf_object_iqr", "bf_object_p90_minus_p10",
                  ...
                  "bf_glcm_entropy", "nucleus_area_cv", "nuclei_spatial_dispersion")) return("log1p")
  if (name %in% c("bf_object_mean_minus_ring_background", "bf_interior_minus_boundary_mean",
                  "bf_glcm_correlation", "nuclei_cell_to_ring_contrast")) return("asinh")
  "identity"
}
...
fit_feature <- function(name, raw, present, reliable, winsor) {
  ...
  if (conditional_present) fit_rows <- fit_rows & present
  if (conditional_reliable) fit_rows <- fit_rows & reliable
  ...
  if (name == "nucleus_absent") {
    ...
    output[!reliable | !is.finite(output)] <- 0
    return(list(
      ...
      conditional_present = FALSE, conditional_reliable = TRUE
    ))
  }
  ...
  if (conditional_present) {
    output[!present | !is.finite(output)] <- 0
  ...
  list(
    ...
    conditional_present = conditional_present, conditional_reliable = conditional_reliable
  )
}
...
prune_brightfield <- function(processed, threshold) {
  retained <- character()
  removed <- list()
  for (name in intersect(BRIGHTFIELD, colnames(processed))) {
    if (!length(retained)) {
    ...
    if (maximum >= threshold) {
      ...
      removed[[length(removed) + 1L]] <- data.frame(
        feature = name, reason = "absolute_spearman_at_or_above_threshold",
...
equalize_blocks <- function(processed, block_map, active_blocks) {
  output <- processed
  rows <- list()
  target <- 1 / length(active_blocks)
  for (block in active_blocks) {
    names <- names(block_map)[block_map == block & names(block_map) %in% colnames(processed)]
    if (!length(names)) fail("V3 active block has no retained features: ", block)
    before <- mean_pairwise_squared_distance(processed[, names, drop = FALSE])
    if (!is.finite(before) || before <= 0) fail("V3 active block has zero geometry: ", block)
...
profile_spec <- function(profile, pruned_bf) {
  blocks <- list(shape = SHAPE, brightfield = BRIGHTFIELD, nuclei = NUCLEI)
  equal <- FALSE
  prune <- FALSE
  if (profile != "v3_zero_aware_unpruned") prune <- TRUE
  if (prune) blocks$brightfield <- pruned_bf
  if (profile %in% c("v3_pruned_block_equal", "v3_pruned_block_equal_plus_nuclei_signal_qc",
                     "v3_minus_shape", "v3_minus_brightfield", "v3_minus_nuclei")) equal <- TRUE
  if (profile == "v3_pruned_block_equal_plus_nuclei_signal_qc") blocks$nuclei <- c(blocks$nuclei, NUCLEI_QC)
  if (profile == "v3_minus_shape") blocks$shape <- NULL
  if (profile == "v3_minus_brightfield") blocks$brightfield <- NULL
  if (profile == "v3_minus_nuclei") blocks$nuclei <- NULL
  list(blocks = blocks, equal = equal)
}
...
recursive_hashes <- function(directory, exclude = character()) {
  ...
  if (any(nzchar(Sys.readlink(paths)))) fail("V3 projection output contains a symlink")
...
verify_existing <- function(output, expected_inputs, implementation_sha) {
  ...
  if (!file.exists(manifest_path) || !file.exists(identity_path)) fail("Existing V3 projection is partial")
  manifest <- jsonlite::fromJSON(manifest_path, simplifyVector = FALSE)
  if (!identical(manifest$schema_version, SCHEMA_VERSION) || !identical(manifest$status, "COMPLETE")) fail("Existing V3 projection manifest is incomplete")
  if (!identical(manifest$inputs, expected_inputs)) fail("Existing V3 projection input identity differs")
  observed <- recursive_hashes(output, c("projection_manifest.json", "projection_generation_identity.json"))
  if (!identical(unlist(manifest$output_file_sha256, use.names = TRUE), observed)) fail("Existing V3 projection artifact set/hash differs")
  ...
  if (!identical(jsonlite::fromJSON(identity_path, simplifyVector = FALSE), expected_identity)) fail("Existing V3 projection generation identity differs")
}
...
main <- function() {
  ...
  if (length(missing)) fail("Missing V3 projection R packages: ", paste(missing, collapse = ","))
  ...
  if (args$check_config) {
    cat("multimodal_cell_state_v3_config=PASS\n")
    cat("primary_profile=", PRIMARY_PROFILE, "\n", sep = "")
    cat("block_equal_target=0.3333333333333333\n")
    return(invisible(0L))
  }
  ...
  for (path in c(unlist(paths), config_path, implementation, expanded)) {
    if (nzchar(Sys.readlink(path))) fail("V3 projection input must not be a symlink: ", path)
  }
  ...
  required_features <- c("cell_id", SHAPE, BRIGHTFIELD, NUCLEI, NUCLEI_QC, "nuclei_measurement_status")
  if (!all(required_cells %in% names(cells))) fail("V3 cells lack identity/audit metadata")
  if (!all(required_features %in% names(features))) fail("V3 merged features lack configured inputs")
  ...
  if (nrow(cells) != expected_n || nrow(features) != expected_n ||
    ...
    fail("V3 development row universe/order differs")
  }
  ...
  ))) fail("V3 Nuclei measurement status is invalid")
  ...
  if (file.exists(output_arg) && !dir.exists(output_arg)) fail("V3 projection output exists and is not a directory")
  output <- normalizePath(output_arg, mustWork = FALSE)
  if (dir.exists(output)) {
    if (!args$overwrite) fail("V3 projection output already exists: ", output)
    verify_existing(output, inputs, implementation_sha)
    cat("v3_projection=", output, "\n", sep = "")
  ...
  present <- features$nuclei_measurement_status == "present_supported"
  reliable <- features$nuclei_measurement_status %in% c("present_supported", "absent_supported")
  winsor <- as.numeric(as_vector(config$preprocessing$winsor_quantiles, "winsor quantiles"))
  all_names <- c(SHAPE, BRIGHTFIELD, NUCLEI, NUCLEI_QC)
  ...
  for (name in all_names) {
    ...
    fitted <- fit_feature(name, raw, present, reliable, winsor)
    if (isTRUE(fitted$keep)) {
      ...
      transform_rows[[length(transform_rows) + 1L]] <- data.frame(
        feature = name, block = if (name %in% SHAPE) "shape" else if (name %in% BRIGHTFIELD) "brightfield" else "nuclei",
  ...
  if (length(missing_required)) fail("Required V3 Shape/Nuclei feature became constant or unavailable: ", paste(missing_required, collapse = ","))
  processed <- processed[, retained_base, drop = FALSE]
  prune_threshold <- as.numeric(config$feature_blocks$brightfield$correlation_pruning$threshold)
  pruning <- prune_brightfield(processed, prune_threshold)
  if (!length(pruning$retained)) fail("V3 Brightfield pruning removed every feature")

  retention <- data.frame(
    feature = all_names,
    block = ifelse(all_names %in% SHAPE, "shape", ifelse(all_names %in% BRIGHTFIELD, "brightfield", "nuclei")),
    transform_retained = all_names %in% retained_base,
    primary_retained = all_names %in% c(SHAPE, pruning$retained, NUCLEI),
    reason = ifelse(!(all_names %in% retained_base), "transform_constant_or_unavailable",
      ifelse(all_names %in% BRIGHTFIELD & !(all_names %in% pruning$retained), "brightfield_absolute_spearman_pruned",
        ifelse(all_names %in% NUCLEI_QC, "sensitivity_profile_only", "retained")))
  )
  write_tsv(retention, file.path(staging, "feature_retention_manifest.tsv"))
  write_tsv(pruning$removed, file.path(staging, "brightfield_correlation_pruning.tsv"))
  ...
  for (profile in GENERATED_PROFILES) {
    specification <- profile_spec(profile, pruning$retained)
    ...
    if (specification$equal) {
      equalized <- equalize_blocks(matrix, block_map, active_blocks)
    ...
    combinations <- if (profile == PRIMARY_PROFILE) expand.grid(
  ...
  for (name in names(audit_metadata)) {
    ...
    batch_rows[[length(batch_rows) + 1L]] <- data.frame(
      ...
      interpretation = if (name == "nuclei_measurement_status") "measurement_status_audit_not_batch" else "known_design_or_batch_audit_not_optimized"
    )
  }
  ...
  equal_pass <- nrow(selected_block) == 3L && all(abs(selected_block$observed_fraction - 1 / 3) <= tolerance)
  status_counts <- as.data.frame(table(features$nuclei_measurement_status), stringsAsFactors = FALSE)
  names(status_counts) <- c("status", "row_count")
  status_counts$fraction <- status_counts$row_count / nrow(features)
  write_tsv(status_counts, file.path(staging, "nuclei_measurement_status_summary.tsv"))
  decision <- list(
    schema_version = "multimodal_cell_state_v3_calibration_decision_v1",
    status = "COMPLETE",
    representation_gate = if (equal_pass) "GO_TO_BLIND_LABEL_MAPPING" else "NO_GO_BLOCK_EQUALIZATION_FAILED",
    ...
    scientific_boundary = paste(
      "UMAP and diagnostic clusters organize multimodal morphology; they are not cell-state labels.",
      "Dead-cell classification requires blinded manual labels and grouped nested cross-validation."
    )
  )
  ...
  manifest <- list(
    ...
    method_version = "sum159_multimodal_cell_state_v3", inputs = inputs,
    row_count = nrow(cells), primary_profile = PRIMARY_PROFILE,
    selected_run_id = selected_run,
    selected_feature_columns = as.list(c(SHAPE, pruning$retained, NUCLEI)),
    brightfield_pruned_columns = as.list(setdiff(BRIGHTFIELD, pruning$retained)),
    zero_aware_policy = config$nuclei_zero_aware,
  ...
  if (!identical(sha256_file(implementation), implementation_sha)) fail("V3 projection implementation changed during execution")
  if (file.exists(output) || dir.exists(output)) fail("V3 projection output appeared during staging")
  if (!file.rename(staging, output)) fail("Failed to install V3 projection atomically")
  cat("v3_projection=", output, "\n", sep = "")
  cat("primary_profile=", PRIMARY_PROFILE, "\n", sep = "")
