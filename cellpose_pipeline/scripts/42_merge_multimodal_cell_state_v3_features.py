#!/usr/bin/env python3
"""Merge immutable per-field V3 features onto one frozen development universe."""
...
SCHEMA_VERSION = "multimodal_cell_state_v3_feature_merge_v1"
FIELD_SCHEMA_VERSION = "multimodal_cell_state_v3_feature_generation_v1"
...
def artifact_hashes(directory: Path, exclude: set[str] | None = None) -> dict[str, str]:
    ...
    for path in sorted(directory.rglob("*")):
        if path.is_symlink():
            raise ValueError(f"V3 feature merge contains a symlink: {path}")
        if path.is_file():
...
def verify_existing(output: Path, expected_identity: dict[str, Any]) -> None:
    ...
    if not manifest_path.is_file() or not identity_path.is_file():
        raise ValueError("Existing V3 feature merge is partial")
    manifest = json.loads(manifest_path.read_text(encoding="utf-8"))
    if (
        manifest.get("schema_version") != SCHEMA_VERSION
        or manifest.get("status") != "COMPLETE"
    ):
        raise ValueError("Existing V3 feature merge manifest is not COMPLETE")
    if manifest.get("identity") != expected_identity:
        raise ValueError("Existing V3 feature merge identity differs")
    ...
    if manifest.get("output_file_sha256") != observed:
        raise ValueError("Existing V3 feature merge artifact set/hash differs")
    expected_generation = {
        "schema_version": "multimodal_cell_state_v3_feature_merge_generation_identity_v1",
        "status": "COMPLETE",
    ...
    if json.loads(identity_path.read_text(encoding="utf-8")) != expected_generation:
        raise ValueError("Existing V3 feature merge generation identity differs")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    config_path = canonical_file(args.feature_config, "V3 feature config")
    extractor = canonical_file(args.field_extractor, "V3 field extractor")
    helper = canonical_file(args.feature_helper, "V3 feature helper")
    implementation = canonical_file(Path(__file__), "V3 merge implementation")
    project = json.loads(project_path.read_text(encoding="utf-8"))
    config = json.loads(config_path.read_text(encoding="utf-8"))
    if config.get("schema_version") != CONFIG_SCHEMA_VERSION:
        raise ValueError("Unsupported V3 feature config schema")
    ...
    if not required_cells.issubset(cell_fields):
        raise ValueError(
            f"V3 project cells lack fields: {sorted(required_cells-set(cell_fields))}"
        )
    if len(cells) != args.expected_cell_count:
        raise ValueError(f"V3 development cell count differs: {len(cells)}")
    if any(row["split"] != "development" for row in cells):
        raise ValueError("Heldout cells entered V3 feature merge")
    ordered_ids = [row["cell_id"] for row in cells]
    if any(not value for value in ordered_ids) or len(set(ordered_ids)) != len(
        ordered_ids
    ):
        raise ValueError("V3 project cell IDs are blank or duplicated")

    generation_dirs = sorted(
        {path.parent for path in feature_root.rglob("feature_receipt.json")}
    )
    if not generation_dirs:
        raise ValueError(f"No V3 per-field generations found under {feature_root}")
    ...
    for generation in generation_dirs:
        if generation.is_symlink():
            raise ValueError(f"V3 field generation is a symlink: {generation}")
        ...
        if not features_path.is_file() or features_path.is_symlink():
            raise ValueError(f"V3 field generation lacks features.tsv: {generation}")
        receipt = json.loads(receipt_path.read_text(encoding="utf-8"))
        if (
            receipt.get("schema_version") != FIELD_SCHEMA_VERSION
            or receipt.get("status") != "COMPLETE"
        ):
            raise ValueError(f"V3 field receipt is incomplete: {receipt_path}")
        ...
        for name, expected_hash in expected_hashes.items():
            if inputs.get(name, {}).get("sha256") != expected_hash:
                raise ValueError(
                    f"V3 field receipt {name} identity differs: {receipt_path}"
                )
        if receipt.get("output_file_sha256") != {
            "features.tsv": sha256_file(features_path)
        }:
            raise ValueError(f"V3 field artifact hash differs: {features_path}")
        fields, rows = read_tsv(features_path)
        if feature_fields is None:
            feature_fields = fields
        elif fields != feature_fields:
            raise ValueError("V3 per-field feature headers differ")
        if len(rows) != int(receipt.get("row_count", -1)):
            raise ValueError(f"V3 field row count differs: {features_path}")
        for row in rows:
            cell_id = row.get("cell_id", "")
            if not cell_id or cell_id in by_id:
                raise ValueError(
                    f"V3 field features have blank/duplicate cell_id: {cell_id!r}"
                )
            by_id[cell_id] = row
    ...
    if set(by_id) != set(ordered_ids):
        ...
        raise ValueError(
            f"V3 field/project cell universe differs: missing={missing} unknown={unknown}"
        )
    ...
    for cell, row in zip(cells, merged, strict=True):
        if (
            ...
            raise ValueError(f"V3 merged identity differs for {cell['cell_id']}")
    inputs = {
    ...
    if output.exists():
        if output.is_symlink() or not output.is_dir():
            raise ValueError(
                f"V3 feature merge output is not a real directory: {output}"
            )
        if not args.overwrite:
            raise FileExistsError(output)
        verify_existing(output, inputs)
        print(f"v3_feature_merge={output}")
    ...
    try:
        ...
        if output.exists():
            raise FileExistsError(
                f"V3 feature merge output appeared during staging: {output}"
            )
