...
validate_qos_walltime_limits() {
  ...
  qos_row="$(sacctmgr --noheader --parsable2 show qos "$BROAD_PHENOTYPE_QOS" format=Name,MaxWall 2>/dev/null | awk -F '|' -v qos="$BROAD_PHENOTYPE_QOS" '$1==qos {print; n++} END {if(n!=1) exit 2}')" || {
...
SOURCE_FIELD_MANIFEST_FILE="${SOURCE_FIELD_MANIFEST_FILE:-$FIELD_MANIFEST_DIR/field_manifest.tsv}"
...
FEATURE_WORKER="$SCRIPT_DIR/run_broad_phenotype_feature_array_task.sh"
PREFLIGHT_WORKER="$SCRIPT_DIR/run_broad_phenotype_input_preflight.sh"
PREDICT_WORKER="$SCRIPT_DIR/run_broad_phenotype_predict_array_task.sh"
MODEL_ACCEPTANCE_WORKER="$SCRIPT_DIR/run_broad_phenotype_model_acceptance.sh"
ADAPTER_WORKER="$SCRIPT_DIR/run_broad_phenotype_cpa_adapter.sh"
STAGE_WORKER="$SCRIPT_DIR/run_broad_phenotype_cpa_stage.sh"
FINALIZE_WORKER="$SCRIPT_DIR/run_broad_phenotype_finalize.sh"
for worker in "$PREFLIGHT_WORKER" "$FEATURE_WORKER" "$PREDICT_WORKER" "$MODEL_ACCEPTANCE_WORKER" "$ADAPTER_WORKER" "$STAGE_WORKER" "$FINALIZE_WORKER"; do
  [[ -x "$worker" ]] || { echo "Required SIF-backed worker is unavailable: $worker" >&2; exit 2; }
done
...
cleanup_submission_staging() {
  if [[ -n "$submission_staging_dir" && -d "$submission_staging_dir" ]]; then
    rm -f -- "$submission_staging_dir/field_tasks.txt" "$submission_staging_dir/hpc_container_identity.json"
...
project_git_tree="$(git -C "$PROJECT_DIR" rev-parse 'HEAD^{tree}')"
project_git_status="$(git -C "$PROJECT_DIR" status --porcelain --untracked-files=all)"
[[ -z "$project_git_status" ]] || {
  echo "PROJECT_DIR must be a clean immutable checkout before broad-phenotype submission" >&2
  exit 2
}
...
cpu_sbatch() {
  env -u SBATCH_QOS -u SBATCH_PARTITION -u SBATCH_ACCOUNT -u SBATCH_GRES -u SBATCH_TRES_PER_NODE -u SBATCH_GPUS -u SBATCH_GPUS_PER_NODE -u SBATCH_GPUS_PER_TASK -u SBATCH_NODELIST -u SBATCH_NODES -u SBATCH_CONSTRAINT -u SBATCH_EXCLUDE -u SBATCH_HOSTLIST sbatch --parsable "$@"
}
submit_job() {
...
if [[ "$RESUME_STAGE" == "phase-a" ]]; then
  ...
  disarm_bootstrap_cleanup
  preflight_job="$(submit_job input-preflight --job-name bp_input_preflight --cpus-per-task "$PREFLIGHT_CPUS" --mem "$PREFLIGHT_MEM" --time "$PREFLIGHT_TIME" "${base_args[@]}" "${dependency_args[@]}" --export=ALL,RESOLVED_FIELD_MANIFEST_DIR="$RESOLVED_FIELD_MANIFEST_DIR",SOURCE_FIELD_MANIFEST_FILE="$SOURCE_FIELD_MANIFEST_FILE" "$PREFLIGHT_WORKER")"
  printf 'input_preflight\t%s\t%s\n' "$preflight_job" "${DEPENDENCY_JOB_ID:-none}" >> "$phase_a_dag"
  feature_job="$(submit_job feature --job-name bp_feature --cpus-per-task "$FEATURE_CPUS" --mem "$FEATURE_MEM" --time "$FEATURE_TIME" --array "1-${N_TASKS}%${FEATURE_MAX_CONCURRENT}" "${base_args[@]}" --dependency "afterok:$preflight_job" --export=ALL "$FEATURE_WORKER")"
  printf 'feature\t%s\t%s\n' "$feature_job" "$preflight_job" >> "$phase_a_dag"
  adapter_job="$(submit_job adapter --job-name bp_adapter --cpus-per-task "$ADAPTER_CPUS" --mem "$ADAPTER_MEM" --time "$ADAPTER_TIME" "${base_args[@]}" --dependency "afterok:$feature_job" --export=ALL "$ADAPTER_WORKER")"
  printf 'adapter_projection\t%s\t%s\n' "$adapter_job" "$feature_job" >> "$phase_a_dag"
  validate_job="$(submit_job validate --job-name bp_validate --cpus-per-task "$VALIDATE_CPUS" --mem "$VALIDATE_MEM" --time "$VALIDATE_TIME" "${base_args[@]}" --dependency "afterok:$adapter_job" --export=ALL,CPA_STAGE=validate,CPA_VALIDATE_STAGE=umap,CPA_STAGE_MODE= "$STAGE_WORKER")"
  printf 'validate\t%s\t%s\n' "$validate_job" "$adapter_job" >> "$phase_a_dag"
  umap_job="$(submit_job umap --job-name bp_umap --cpus-per-task "$UMAP_CPUS" --mem "$UMAP_MEM" --time "$UMAP_TIME" "${base_args[@]}" --dependency "afterok:$validate_job" --export=ALL,CPA_STAGE=umap,CPA_STAGE_MODE= "$STAGE_WORKER")"
  printf 'umap\t%s\t%s\n' "$umap_job" "$validate_job" >> "$phase_a_dag"
  annotate_job="$(submit_job annotate --job-name bp_annotate --cpus-per-task "$ANNOTATE_CPUS" --mem "$ANNOTATE_MEM" --time "$ANNOTATE_TIME" "${base_args[@]}" --dependency "afterok:$umap_job" --export=ALL,CPA_STAGE=annotate,CPA_STAGE_MODE= "$STAGE_WORKER")"
...
if [[ "$RESUME_STAGE" == "predict-sharded" ]]; then
  ...
  FEATURE_MANIFEST="${FEATURE_MANIFEST:-$SHADOW_ROOT/workflow_status/feature_inventory/${BRANCH}_feature_manifest.tsv}"
  PREDICTION_ROOT="${PREDICTION_ROOT:-$SHADOW_ROOT/predictions/sharded}"
  ...
  acceptance_job="$(submit_job model-acceptance --job-name bp_model_acceptance --cpus-per-task "$MODEL_ACCEPTANCE_CPUS" --mem "$MODEL_ACCEPTANCE_MEM" --time "$MODEL_ACCEPTANCE_TIME" "${base_args[@]}" "${dependency_args[@]}" --export=ALL "$MODEL_ACCEPTANCE_WORKER")"
  prediction_job="$(submit_job predict-sharded --job-name bp_predict_sharded --cpus-per-task "$PREDICT_CPUS" --mem "$PREDICT_MEM" --time "$PREDICT_TIME" --array "1-${prediction_tasks}%${PREDICT_MAX_CONCURRENT}" "${base_args[@]}" --dependency "afterok:$acceptance_job" --export=ALL "$PREDICT_WORKER")"
...
if [[ "$RESUME_STAGE" == "input-preflight" ]]; then
  ...
  resume_job="$(submit_job input-preflight --job-name bp_input_preflight --cpus-per-task "$PREFLIGHT_CPUS" --mem "$PREFLIGHT_MEM" --time "$PREFLIGHT_TIME" "${base_args[@]}" "${dependency_args[@]}" --export=ALL,RESOLVED_FIELD_MANIFEST_DIR="$RESOLVED_FIELD_MANIFEST_DIR",SOURCE_FIELD_MANIFEST_FILE="$SOURCE_FIELD_MANIFEST_FILE" "$PREFLIGHT_WORKER")"
...
if [[ "$RESUME_STAGE" == "feature-retry" ]]; then
  ...
  [[ ! -e "$resume_submission" ]] || { echo "Immutable resume submission already exists: $resume_submission" >&2; exit 2; }
  resume_job="$(submit_job feature-retry --job-name bp_feature_retry --cpus-per-task "$FEATURE_CPUS" --mem "$FEATURE_MEM" --time "$FEATURE_TIME" --array "1-${retry_task_count}%${FEATURE_MAX_CONCURRENT}" "${base_args[@]}" "${dependency_args[@]}" --export=ALL "$FEATURE_WORKER")"
...
if [[ "$RESUME_STAGE" == "adapter" ]]; then
  resume_worker="$ADAPTER_WORKER"; resume_export="ALL"
  ...
  resume_worker="$STAGE_WORKER"; resume_export="ALL,CPA_STAGE=validate,CPA_VALIDATE_STAGE=umap,CPA_STAGE_MODE=$CPA_STAGE_MODE"
  ...
  resume_worker="$STAGE_WORKER"; resume_export="ALL,CPA_STAGE=$RESUME_STAGE,CPA_STAGE_MODE=$CPA_STAGE_MODE"
else

case "$RESUME_STAGE" in
  annotation-import) : "${CPA_SUBMISSION:?CPA_SUBMISSION is required}"; export CPA_SUBMISSION ;;
  review-build) : "${CPA_ANNOTATION_IMPORT_DIR:?CPA_ANNOTATION_IMPORT_DIR is required}"; export CPA_ANNOTATION_IMPORT_DIR ;;
  review-import)
    ...
    ;;
  train) : "${CPA_REVIEWED_LABELS:?CPA_REVIEWED_LABELS is required}"; export CPA_REVIEWED_LABELS ;;
  predict|report) : "${CPA_MODEL_DIR:?CPA_MODEL_DIR is required}"; export CPA_MODEL_DIR ;;
  finalize)
    : "${PREDICTION_DIR:?PREDICTION_DIR is required}"
    export PREDICTION_DIR
    if [[ -n "${MODEL_DIR:-}" ]]; then export MODEL_DIR; fi
    ;;
  finalize-sharded)
    FEATURE_MANIFEST="${FEATURE_MANIFEST:-$SHADOW_ROOT/workflow_status/feature_inventory/${BRANCH}_feature_manifest.tsv}"
    PREDICTION_ROOT="${PREDICTION_ROOT:-$SHADOW_ROOT/predictions/sharded}"
...
if [[ "$RESUME_STAGE" == "finalize" || "$RESUME_STAGE" == "finalize-sharded" ]]; then
  ...
  resume_export="ALL,CPA_STAGE=$RESUME_STAGE,CPA_STAGE_MODE=$CPA_STAGE_MODE"
fi
fi
resume_submission="$SHADOW_ROOT/workflow_status/resume_submission_${RESUME_STAGE}_${RUN_STAMP}.tsv"
[[ ! -e "$resume_submission" ]] || { echo "Immutable resume submission already exists: $resume_submission" >&2; exit 2; }
resume_job="$(submit_job "$RESUME_STAGE" --job-name "bp_${RESUME_STAGE//-/_}" --cpus-per-task "$resume_cpus" --mem "$resume_mem" --time "$resume_time" "${base_args[@]}" "${dependency_args[@]}" --export="$resume_export" "$resume_worker")"
